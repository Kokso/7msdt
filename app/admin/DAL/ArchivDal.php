...
class ArchivDal {
    ...
    public function InsertArchive($arr){
        $insCount = dibi::query('INSERT INTO [ms_archive]', $arr);
        if ($insCount != 1) {
            throw new Exception("Create of archive failed");
        }
        return dibi::insertId();
    }
    
    public function CopyClasses($archiveId) {
        dibi::query("INSERT INTO ms_archivedclasses(classId, `name`, archiveId) SELECT id, `name`, $archiveId FROM ms_classes WHERE `type`=1");       
    }
    
    public function UpdateClasses($archiveId) {
        dibi::query("UPDATE `ms_events` SET `archiveId`=$archiveId WHERE `archiveId` IS NULL");       
    }
