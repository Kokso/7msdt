...
if ($authen->IsLogged()) {
    if (count($_FILES) == 1) {
        ...
        try {
            $targetPath = '/www/data/zverejnovanie/';
            $file->Save($rootDir.$targetPath, NULL, array('pdf'));
            
            Json::Send(new ErrorResponse(ErrorResponse::$SuccessErr, (object) array('filePath' => $targetPath . $file->GetName())));
        } catch (Exception $ex) {
            switch ($ex->getCode()) {
                case File::$ErrFileType:
                    Json::Send(new ErrorResponse(ErrorResponse::$OperationErr, "Súbor musí byť vo formáte PDF"));
                    break;
                case File::$ErrSave:
